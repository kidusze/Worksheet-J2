...
4. No the season does not change at all because when the change_season(season) is called there is no reassaigining of the variable(or it's address, A100) from 'winter' to that of spring. This is because the method only modifies a local copy of the reference to the string.
